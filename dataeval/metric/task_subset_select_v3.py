...
Upgraded selector for per-task subsets under fixed budget m=ceil(ratio*N):
  ...
  (A) BEST subset: maximize L_raw
      best_init = max( multi-start greedy , random-max search )
      best_final = 2-swap local search refinement on best_init
...
Example:
  python -m dataeval.metric.select_subset_cli \
    ...
    --two_swap_rounds 5 \
    --two_swap_trials_per_round 2000 \
    --seed 0 \
...
def _ensure_2d_features(X: np.ndarray) -> np.ndarray:
    ...
    if X.ndim < 2:
        raise ValueError(f"features must have ndim>=2, got shape={X.shape}")
    if X.ndim == 2:
        return X
    return X.reshape(X.shape[0], -1)


def subset_task_length_from_demo_lengths(
...
def greedy_select_task_subset(
    ...
    *,
    ...
    def L_of(S: List[int]) -> float:
        return evaluate_subset_L(
            X, demo_lengths_task, S,
            beta=beta,
            intra_sim=intra_sim,
            adaptive_tau=adaptive_tau,
            tau_intra=tau_intra,
...
# =========================================================
# 4) 2-swap local search refinement
# =========================================================
def two_swap_local_search_max(
    X: np.ndarray,
    demo_lengths_task: np.ndarray,
    init_idx: List[int] | np.ndarray,
    *,
    beta: float = 0.5,
    intra_sim: str = "exp",
    adaptive_tau: bool = True,
    ...
    length_penalty: bool = True,
    max_rounds: int = 5,
    max_trials_per_round: Optional[int] = 2000,
    seed: int = 0,
    eps: float = 1e-12,
) -> Tuple[List[int], float, int]:
    """
    Maximize L_raw via 2-swap local search:
      S' = S - {o1,o2} + {i1,i2}

    Uses sampled neighborhood if full neighborhood is too large.

    Returns:
      refined_idx, refined_L, total_moves_evaluated
    """
    X = _ensure_2d_features(X)
    demo_lengths_task = np.asarray(demo_lengths_task, dtype=np.float32)
    rng = np.random.default_rng(seed)
    ...
    N = int(X.shape[0])
    S = sorted(set(int(x) for x in init_idx))
    S_set = set(S)

    cur_L = evaluate_subset_L(
        X, demo_lengths_task, S,
        beta=beta, intra_sim=intra_sim,
        adaptive_tau=adaptive_tau, tau_intra=tau_intra,
        tau_floor=tau_floor, tau_ceiling=tau_ceiling,
    ...
    for _round in range(int(max_rounds)):
        U = [i for i in range(N) if i not in S_set]
        if len(S) < 2 or len(U) < 2:
            break

        out_pairs = [(S[a], S[b]) for a in range(len(S)) for b in range(a + 1, len(S))]
        in_pairs = [(U[a], U[b]) for a in range(len(U)) for b in range(a + 1, len(U))]

        total_moves = len(out_pairs) * len(in_pairs)
        best_move = None
        best_L = cur_L

        if max_trials_per_round is None or total_moves <= max_trials_per_round:
            move_iter = []
            for op in out_pairs:
                for ip in in_pairs:
                    move_iter.append((op, ip))
        else:
            move_iter = None

        if move_iter is not None:
            for (o1, o2), (i1, i2) in move_iter:
                cand = [x for x in S if x != o1 and x != o2] + [i1, i2]
                cand = sorted(cand)

                L_new = evaluate_subset_L(
                    X, demo_lengths_task, cand,
                    beta=beta, intra_sim=intra_sim,
                    adaptive_tau=adaptive_tau, tau_intra=tau_intra,
                    tau_floor=tau_floor, tau_ceiling=tau_ceiling,
                    length_penalty=length_penalty,
            ...
            for _ in range(int(max_trials_per_round)):
                o1, o2 = rng.choice(S, size=2, replace=False)
                i1, i2 = rng.choice(U, size=2, replace=False)
                cand = [x for x in S if x != int(o1) and x != int(o2)] + [int(i1), int(i2)]
                cand = sorted(cand)
    ...
    return S, float(cur_L), int(total_evals)


# =========================================================
# 5) Build filtered npz (global mask)
# =========================================================
...
def parse_args():
    p = argparse.ArgumentParser(
        "Upgraded: BEST=max(greedy,many-random)+2swap; BAD=random-min. Export npz + report"
    )
    ...
    # ---- 2-swap refinement knobs ----
    p.add_argument("--two_swap_rounds", type=int, default=5,
                   help="How many rounds of 2-swap local search for BEST refinement")
    p.add_argument("--two_swap_trials_per_round", type=int, default=2000,
                   help="If full 2-swap neighborhood is larger than this, sample this many moves per round")
    p.add_argument("--two_swap_seed_offset", type=int, default=777777)
...
def main():
    ...
    print(
        "[INFO] L_raw-only selection with 2-swap refinement on BEST. "
        f"Recorded but NOT used here: task_knn={args.task_knn}, "
        f"task_temp={args.task_temp}, alpha={args.alpha}, pi_scale={args.pi_scale}"
    )
    ...
    report: Dict[str, Any] = {
        ...
        "meta": {
            ...
            # 2-swap
            "two_swap_rounds": int(args.two_swap_rounds),
            "two_swap_trials_per_round": int(args.two_swap_trials_per_round),
            "two_swap_seed_offset": int(args.two_swap_seed_offset),
            # random-min
    ...
    for r in ratios:
        ...
        for g in task_groups:
            tid = int(g["task_id"])
            X = _ensure_2d_features(np.asarray(g["features"], dtype=np.float32))
            ...
            # ---------- choose BEST init = max(greedy, randmax) ----------
            if randmax_L >= greedy_L:
                best_init_src = "random_max"
                best_init_idx_local = randmax_idx_local
                best_init_L = randmax_L
            else:
                best_init_src = "greedy_multistart"
                best_init_idx_local = greedy_idx_local
                best_init_L = greedy_L

            # ---------- refine BEST init via 2-swap local search ----------
            best_idx_local, best_L, two_swap_evals = two_swap_local_search_max(
                X, demo_lens_task, best_init_idx_local,
                beta=args.beta,
                intra_sim=args.intra_sim,
                adaptive_tau=adaptive_tau,
                ...
                length_penalty=length_penalty,
                max_rounds=int(args.two_swap_rounds),
                max_trials_per_round=(
                    None if int(args.two_swap_trials_per_round) <= 0
                    else int(args.two_swap_trials_per_round)
                ),
                seed=args.seed + args.two_swap_seed_offset + tid * 1009,
            )

            best_src = "2swap_refined_from_" + best_init_src
            ...
            per_task[str(tid)] = {
                ...
                "best": {
                    ...
                    "best_init_source": best_init_src,
                    "best_init_L": float(best_init_L),
                    "best_init_selected_local_indices": [int(x) for x in best_init_idx_local],
                    ...
                    "two_swap_evaluated": int(two_swap_evals),
                    "two_swap_gain_over_init": float(best_L - best_init_L),
                },
                "randommin": {
            ...
            if args.debug:
                print(
                    f"[Task {tid}] N={N} m={m} "
                    f"greedyL={greedy_L:.6f} randmaxL={randmax_L:.6f} "
                    f"init({best_init_src})={best_init_L:.6f} "
                    f"2swapL={best_L:.6f} gain={best_L - best_init_L:.6f} "
